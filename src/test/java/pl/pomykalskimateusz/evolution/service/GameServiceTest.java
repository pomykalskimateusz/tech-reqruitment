...
public class GameServiceTest {
    ...
    public void should_throw_not_found_user() {
        assertThrows(UserNotFound.class, () -> gameService.playFreeGame((long) -1, 5));
    }

    @Test
    public void should_throw_invalid_bet_amount_for_too_large_value_in_free_game() {
        assertThrows(InvalidBetAmountError.class, () -> gameService.playFreeGame(user.getId(), 11));
    }

    @Test
    public void should_throw_invalid_bet_amount_for_to_small_value_in_free_game() {
        assertThrows(InvalidBetAmountError.class, () -> gameService.playFreeGame(user.getId(), 0));
    }

    @Test
    public void should_throw_invalid_bet_amount_for_too_large_value_in_cash_game() {
        assertThrows(InvalidBetAmountError.class, () -> gameService.playCashGame(user.getId(), 11));
    }

    @Test
    public void should_throw_invalid_bet_amount_for_to_small_value_in_cash_game() {
        assertThrows(InvalidBetAmountError.class, () -> gameService.playCashGame(user.getId(), 0));
    }

    @Test
    public void should_return_empty_games_state_for_non_existing_user() {
        List<GameEntity> games = gameService.fetchUserGames((long) -1);
    ...
    public void should_return_games_for_existing_user_played_games() {
        gameService.playFreeGame(user.getId(), 5);
    ...
    @Test
    public void should_return_game_by_id() {
        gameService.playFreeGame(user.getId(), 5);
        List<GameEntity> games = gameService.fetchUserGames(user.getId());
        ...
        assertEquals(game.getWinAmount(), games.get(0).getWinAmount(), 0);
        assertEquals(game.getBalance(), games.get(0).getBalance(), 0);
